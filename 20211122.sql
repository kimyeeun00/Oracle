...
-- 16. 이메일에서 뒤에서 한 글자를 가지고오고, 또 이메일에서 뒤에서부터 2글자를 가지고오며, 이메일에서 뒤에서 3번부터 2글자를 출력하세요.
...
select * from employees where last_name like '%on'; -- 답

-- 10. 성의 뒤에 세번째 글자가 so인 사람을 출력하시오.
select * from employees where substr(last_name,-3) like 'so%';
select * from employees where last_name like '%so_'; -- 답
...
select * from employees where to_char(hire_date,'yyyy') >= '2002'; -- 답 
...
select * from job_history where trunc((end_date-start_date)/365) >= 8; -- 틀림 

select * from employees
where (sysdate-hire_date) / 365 >= 8; -- 답
...
select *  from employees where hire_date = min(hire_date) or hire_date =  max(hire_date); -- 안된당
select min(hire_date), max(hire_date) from employees; -- 답 
...
select * from employees where salary BETWEEN 3000 and 6000; -- 답 

-- 45. 사원의 급여가 3000에서 6000 사이의 급여를 받지 못하는 사원을 출력하시오.
select * from employees where salary < 3000 or salary > 6000; 
select * from employees where salary not BETWEEN 3000 and 6000; -- 답
